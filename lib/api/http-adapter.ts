    ...
    problems: {
      ...
      saveStatement: async (id, input) => {
        const data = await request<ProblemStatementResponse>(`/api/v1/problems/${id}/statement`, {
          ...
          method: "POST",
          headers: { "content-type": "application/json" },
          // 标题由服务端写入 problem.title，不再是题面入参。
          body: JSON.stringify({
      ...
      uploadTestcases: async (id, input) => {
        ...
        const data = await request<UploadedTestcaseSetResponse>(`/api/v1/problems/${id}/testcase-sets`, {
          accessToken: options.accessToken,
          method: "POST",
          body: form,
        });
        return mapUploadedTestcaseSet(data);
      },
