...
- **The nav carries the same lockup, and both of its tiers are unconditional.** The header's brand block is the name with the category word beneath it. That second line used to be `hidden` below 640px — measured at a 390px viewport its box was `0×0`, so a phone's header read `Sundial` and nothing else, which is the same failure the rename fixed in the tab. It is set in the display face, sentence case, rather than in the eyebrow treatment: the eyebrow (mono, uppercase, `0.15em`) marks section names, units and axis ticks, and an identity line is none of those. It stays two lines rather than collapsing to one — at 390px the header's horizontal budget is already spent (the primary nav is squeezed into a scrollable strip), so an inline lockup would take that width out of the nav. The header's own gaps narrow below 640px (`gap-2 sm:gap-5`) to pay for the second line: `Online Judge` is 27px wider than the wordmark above it, and that 27px has to come from somewhere. Measured at 390px, this nets **+36px** of visible nav in Chinese (62 → 98) and **-18px** in English (62 → 44), because the Chinese category is exactly as wide as the name and the English one is not.
